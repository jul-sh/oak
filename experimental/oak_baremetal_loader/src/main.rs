...
impl oak_idl::AsyncHandler for Client {
    async fn invoke(&self, request: oak_idl::Request) -> Result<Vec<u8>, oak_idl::Status> {
...
/// Singleton responsible for sending requests, and receiving responses over the underlyign
...
impl BaremetalCommunicationChannel {
    ...
    pub fn listen(
        &mut self,
        receiver: bmrng::UnboundedRequestReceiver<Vec<u8>, Result<Vec<u8>, oak_idl::Status>>,
    ) {
        tokio::spawn(async move {
            while let Ok((request, response_dispatcher)) = request_receiver.recv().await {
                ...
                // TODO(#2848): Implement message prioritization, and non sequential communication.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    communication_channel.listen(request_receiver);

    let client = schema::TrustedRuntimeAsyncClient::new(Client { request_dispatcher });
    ...
    let server_future = server::server("127.0.0.1:8080".parse()?, client.clone());
